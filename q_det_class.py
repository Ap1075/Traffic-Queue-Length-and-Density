...
class q_lenner():
	...
	def visualize(self, frame, stat, dyn):
		...
		lineType               = 2
		if self.queue_ind:			
			cv2.putText(frame,"Queue Length: {0} metres".format(self.medd), (1000,800), font, 1.5, (255,0,255), 3)
	...
	def corns_neighbourhood(self, feats):
		...
		for i,corner in enumerate(feats):
			x,y = corner.ravel()
			corn_neigh.append([y-1,y+1, x-1,x+1])  # taking 3x3 neighbourhood around the corner pixel
		return corn_neigh
		...
		if self.old_frame is not None:
			...
			if ((len(stat) > 10) or (len(stat)>len(dyn) and len(dyn)<5)):						## after false positives are gone, a simpler if.
				...
				if self.stat_greater_count > 10:
					...
					self.medd = round(self.q_measure(M, [stat[0][1], stat[0][0],1], [stat[-2][1], stat[-2][0],1]), 3)
